class MultipleUploader {
  ...
  init({
    ...
    if (!this.#multipleUploader)
      // check if the end user didnt write the multiple uploader div
      throw new Error("The multiple uploader element doesnt exist");

    if (!form)
      // check if there is no form with this selector
      throw new Error(
        "We couldn't find a form with this selector: " + formSelector
      );
    ...
    if (document.getElementById("max-upload-number"))
      document.getElementById(
        "max-upload-number"
      ).innerHTML = `Upload up to ${maxUpload} files`;

    // create multiple file input and make it hidden
    this.#$imagesUploadInput.type = "file";
    this.#$imagesUploadInput.name = "UploadedImages";
    this.#$imagesUploadInput.id = "UploadedImages";
    ...
    this.#multipleUploader.addEventListener("click", (e) => {
      if (
        e.target.className === "multiple-uploader" ||
        e.target.className === "mup-msg" ||
        e.target.className === "mup-main-msg"
      )
    ...
    this.#$imagesUploadInput.addEventListener("change", function () {
      if (this.files.length > 0) {
        self.#multipleUploader
          .querySelectorAll(".image-container")
          .forEach((image) => image.remove()); // clear the previous rendered images
        self.#multipleUploader
          .querySelector(".mup-msg")
          .style.setProperty("display", "none"); // hide the hint texts inside drop zone
        ...
        const uploadedImagesCount =
          this.files.length > maxUpload ? maxUpload : this.files.length;
        ...
        for (let index = 0; index < uploadedImagesCount; index++) {
          ...
          const isImageSizeAcceptable = self.#checkImageSize(
            index,
            imageSize,
            maxSize,
            "MB"
          ...
          self.#multipleUploader.innerHTML += `
                  <div class="image-container" data-image-index="${index}" id="mup-image-${index}" data-acceptable-image="${+isImageSizeAcceptable}" >
                      <div class="image-size"> ${
                        imageSize["size"] + " " + imageSize["unit"]
                      } </div>
                      ${
                        !isImageSizeAcceptable
                          ? `<div class="exceeded-size"> greater than ${maxSize} MB </div>`
                          : ""
                      }
          ...
          if (!isImageSizeAcceptable) unAcceptableImagesIndices.push(index);
        }

        unAcceptableImagesIndices.forEach((index) =>
          self.#removeFileFromInput(index, false)
        );
      }
    });
  ...
  //      // clicked on remove pseudo element
  //      const imageIndex = e.target.getAttribute(`data-image-index`);
  //      const imageIsAcceptable = e.target.getAttribute(
  //        `data-acceptable-image`
  //      );

  //      e.target.remove(); // remove the html element from the dom

  //      if (+imageIsAcceptable) self.#removeFileFromInput(imageIndex);
  ...
  clear() {
    ...
    this.#multipleUploader
      .querySelectorAll(".mup-msg")
      .forEach((msg) => msg.style.setProperty("display", "flex"));
  ...
  #removeFileFromInput(deletedIndex) {
    ...
    for (const [index, file] of Object.entries(
      this.#$imagesUploadInput.files
    )) {
      if (index != deletedIndex) dt.items.add(file);
    }
  ...
  #checkImageSize(imageIndex, imageSize, maxSize) {
    return (
      imageSize["unit"] !== "MB" ||
      (imageSize["unit"] === "MB" && imageSize["size"] <= maxSize)
  ...
  #bytesToSize(bytes) {
    ...
    else return { size: (bytes / 1024 ** i).toFixed(1), unit: sizes[i] };
  }
}
