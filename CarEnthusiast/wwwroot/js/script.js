...
$document.ready(function () {
        ...
        for (j = 0; j < respTabsArr.length; j++) {
          responsiveTabsItem.find(respTabsArr[j].toString()).on(
            ...
            $.proxy(function (event) {
              var $this = $(this),
                carouselObj = $this
                  .find(".resp-tab-content-active .owl-carousel")
                  .owlCarousel()
                  .data("owlCarousel");
  ...
  if (plugins.rdNavbar.length) {
    ...
    (function () {
      var tmpLinks = plugins.rdNavbar.find("a");
      for (var i = 0; i < tmpLinks.length; i++) {
        if (/^#./.test($(tmpLinks[i]).attr("href"))) {
          tmpLinks[i].addEventListener("click", function () {
            document
              .querySelector(".rd-navbar-nav-wrap")
              .classList.toggle("active");
            document
              .querySelector(".rd-navbar-toggle")
              .classList.toggle("active");
          });
        }
      }
    ...
    for (i = 0; i < plugins.swiper.length; i++) {
      ...
      swiperSlide
        ...
        .swiper({
          ...
          simulateTouch: s.attr("data-simulate-touch")
   ...
   *   Scroll down
   * */
  if (plugins.anchorScroll.length) {
    var i;
    $(".anchor-link a").click(function () {
      $("html, body").animate(
        {
          scrollTop:
            $(this).closest("section").next("section").find(".title").offset()
              .top -
            250 +
            "px",
        },
